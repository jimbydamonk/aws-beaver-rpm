...
%define lname beaver
%define version 30
%define unmangled_version 30
%define unmangled_version 30
%define release 1

%global bindir %{_bindir}
%global confdir %{_sysconfdir}/%{lname}
%global confddir %{_sysconfdir}/%{lname}/conf.d
%global lockfile %{_localstatedir}/lock/subsys/%{lname}
%global logdir %{_localstatedir}/log/%{lname}
%global piddir %{_localstatedir}/run/%{lname}
%global sysconfigdir %{_sysconfdir}/sysconfig
%global sincedb %{_localstatedir}/%{lname}
...
Source1: beaver.logrotate
Source2: beaver.init
Source3: beaver.sysconfig
Source4: beaver.conf 
License: LICENSE.txt
...
Prefix: %{_prefix}
BuildArch: noarch
Vendor: Jose Diaz-Gonzalez <support@savant.be>
...
%{__mkdir} -p %{buildroot}%{confddir}
%{__install} -m 755 %{SOURCE4} %{buildroot}%{confdir}

# Logs
%{__mkdir} -p %{buildroot}%{logdir}
%{__install} -D -m 644 %{SOURCE1} %{buildroot}%{_sysconfdir}/logrotate.d/%{lname}
...
%{__install} -m 755 %{SOURCE2} %{buildroot}%{_initddir}/%{lname}
%{__install} -m 644 %{SOURCE3} %{buildroot}%{sysconfigdir}/%{lname}

%{__mkdir} -p %{buildroot}%{sincedb}

%{__sed} -i \
   -e "s|@@@NAME@@@|%{lname}|g" \
   -e "s|@@@DAEMON@@@|%{bindir}|g" \
   -e "s|@@@CONFDIR@@@|%{confdir}/beaver.conf|g" \
   -e "s|@@@CONFDDIR@@@|%{confddir}|g" \
   -e "s|@@@LOCKFILE@@@|%{lockfile}|g" \
   -e "s|@@@LOGDIR@@@|%{logdir}|g" \
   -e "s|@@@PIDDIR@@@|%{piddir}|g"\
   %{buildroot}%{_initddir}/%{lname}
...
%{__sed} -i \
   ...
   -e "s|@@@CONFDDIR@@@|%{confddir}|g" \
   -e "s|@@@LOGDIR@@@|%{logdir}|g" \
   %{buildroot}%{sysconfigdir}/%{lname}


%{__sed} -i \
   -e "s|@@@DATADIR@@@|%{sincedb}|g" \
   %{buildroot}%{confdir}/%{lname}.conf
...
%pre
# create  group
if ! getent group beaver >/dev/null; then
        groupadd -r beaver
fi

# create  user
if ! getent passwd beaver >/dev/null; then
        useradd -r -g beaver \
            -s /sbin/nologin beaver 
fi

%post
/sbin/chkconfig --add beaver

%preun
if [ $1 -eq 0 ]; then
  /sbin/service beaver stop >/dev/null 2>&1
  /sbin/chkconfig --del beaver
fi
